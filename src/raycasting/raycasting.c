...
void	cast_ray(t_data *data, t_player *player, t_ray *ray)
{
	...
	while (next_horz_touch_x >= 0 && next_horz_touch_x <= (15 * TILE_SIZE) && next_horz_touch_y >= 0 && next_horz_touch_y <= (TILE_SIZE * data->rows))
	{
	...
	// increment x-step and y_step until we find a wall
	while (next_vert_touch_x >= 0 && next_vert_touch_x <= (15 * TILE_SIZE) && next_vert_touch_y >= 0 && next_vert_touch_y <= (TILE_SIZE * data->rows))
	{
